...
class DomainAPIError(Exception):
    """Base exception for Domain API errors."""
    pass


class DomainRateLimitError(DomainAPIError):
    """Raised when API rate limit is exceeded."""
    pass


class DomainAuthenticationError(DomainAPIError):
    """Raised when API authentication fails."""
    pass


class DomainDataError(DomainAPIError):
        ...
        try:
            ...
            async with self.session.request(method, url, params=params) as response:
                ...
                if cache_key and response.status == 200:
                    ...
                    logger.debug("Domain API response cached", cache_key=cache_key)
