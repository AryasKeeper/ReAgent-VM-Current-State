...
class WeaviateClient:
    ...
    async def disconnect(self) -> None:
        ...
        if self._client:
            # Weaviate v3 client doesn't have explicit close
            self._client = None
            self._is_connected = False
        ...
        try:
            ...
            if is_ready:
                try:
                    ...
                    logger.warning(f"Failed to get Weaviate metadata: {e}", exc_info=True)
                    # Continue without metadata for health check
