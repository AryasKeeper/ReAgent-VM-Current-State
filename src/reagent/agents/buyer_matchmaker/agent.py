...
class BuyerMatchmakerAgent(BaseReAgentAgent):
    ...
    async def _find_matches_for_buyer(
        ...
        """Find property matches for a specific buyer using semantic matching engine."""
        ...
        # Use the enhanced semantic matching engine
        from .matching_engine import SemanticMatchingEngine
        ...
        property_matches = await semantic_engine.find_property_matches(
            buyer_profile=buyer,
            limit=self.matching_config.max_matches_per_buyer,
            min_score=self.matching_config.min_match_score
        )
        
        # Convert to MatchResult format for backward compatibility
        match_results = []
        for i, prop_match in enumerate(property_matches):
            match_result = MatchResult(
                property_id=prop_match.property_id,
                buyer_id=prop_match.buyer_id,
                match_score=prop_match.match_score,
                match_rank=i + 1,
                match_reasons=[f"Semantic similarity: {prop_match.component_scores.get('vector_similarity', 0):.1%}"],
                match_concerns=[],
                match_explanation=prop_match.explanation,
                price_assessment="fairly_priced",  # This would be enhanced in the semantic engine
                estimated_value=0.0,  # This would be calculated in the semantic engine
                scoring_details=prop_match.component_scores
            )
            match_results.append(match_result)
        
        # Cache results
        match_dicts = [match.__dict__ for match in match_results]
        ...
        self.logger.info(f"Generated {len(match_results)} semantic matches for buyer {buyer_id}")
    ...
    async def _setup_vector_schemas(self) -> None:
        ...
        from reagent.services.vector_db_service import setup_vector_schemas
    ...
    async def _store_buyer_matches(self, buyer_id: str, matches: List[MatchResult]) -> None:
        ...
        from reagent.data.match_repository import store_buyer_matches
    ...
    async def _sync_properties_to_vector_db(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            from reagent.services.vector_db_service import sync_properties_to_vector_db
    ...
    async def _perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                from reagent.services.monitoring_service import get_matching_performance_metrics
