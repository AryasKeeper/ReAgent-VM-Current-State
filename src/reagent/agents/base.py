...
from reagent.core.exceptions import AgentExecutionError, DatabaseConnectionError, CacheError
from reagent.data.models.agent_models import AgentExecution, AgentTask, AgentLog
...
class BaseReAgentAgent(ABC):
    ...
    def __init__(self, config: AgentConfig):
        self.config = config
        self.logger = get_logger(f"agents.{config.name.lower().replace(' ', '_')}")
    ...
    async def _check_service_availability(self, service: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Service health check failed for {service}: {e}", service=service, exc_info=True)
            return False
        except Exception as e:
            logger = get_logger(__name__)
            logger.error(f"Unexpected error during health check for {service}: {e}", service=service, exc_info=True)
            raise AgentExecutionError(self.name, f"Health check failed: {e}") from e
