...
from src.utils.validation import clean_property_description
from src.core.exceptions import ValidationError, AgentExecutionError
    ...
    def _calculate_days_on_market(self, first_listed_date: datetime) -> int:
        ...
        if isinstance(first_listed_date, str):
            try:
                ...
                logger.warning(f"Failed to parse first listed date: {e}", date_value=first_listed_date)
                return 0
            except Exception as e:
                logger.error(f"Unexpected error parsing date: {e}", date_value=first_listed_date, exc_info=True)
                raise ValidationError("date_parsing", first_listed_date, f"Date parsing failed: {e}") from e
