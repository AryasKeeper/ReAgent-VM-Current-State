...
from src.core.exceptions import ValidationError, DatabaseQueryError
import structlog
...
class StatisticalValidator:
    ...
    def _calculate_statistical_power(self, r2: float, n: int, p: int) -> float:
        ...
        try:
            ...
            logger.warning(f"Failed to calculate statistical power: {e}", exc_info=True)
            return 0.5  # Default moderate power
        except Exception as e:
            logger = structlog.get_logger(__name__)
            logger.error(f"Unexpected error calculating statistical power: {e}", exc_info=True)
            raise ValidationError("statistical_power", None, f"Power calculation failed: {e}") from e
