...
class MarketAnomalyDetector:
    ...
    def _generate_anomaly_opportunity_description(self, anomalies: List[MarketAnomaly]) -> str:
        ...
        try:
            ...
            logger.warning(f"Failed to generate anomaly description: {e}", 
                          anomaly_count=len(anomalies) if anomalies else 0, exc_info=True)
            return f"Property exhibits {len(anomalies) if anomalies else 0} market anomalies suggesting potential opportunity."
