...
from ...core.exceptions import ComplianceError, ValidationError, AgentExecutionError
...
class ComplianceMonitor:
    ...
    def _is_public_data_only(self, opportunity: OffMarketOpportunity) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to validate data source legitimacy: {e}", 
                        opportunity_id=getattr(opportunity, 'id', 'unknown'), exc_info=True)
            return False  # Fail-safe: assume non-compliant if validation fails
    ...
    def _has_personal_info_protection(self, opportunity: OffMarketOpportunity) -> bool:
        ...
        try:
            ...
            logger = structlog.get_logger(__name__)
            logger.warning(f"Failed to check personal info protection: {e}", 
                          opportunity_id=getattr(opportunity, 'id', 'unknown'), exc_info=True)
    ...
    def _is_ethically_sourced(self, opportunity: OffMarketOpportunity) -> bool:
        ...
        try:
            ...
            logger = structlog.get_logger(__name__)
            logger.warning(f"Failed to check ethical sourcing: {e}", 
                          opportunity_id=getattr(opportunity, 'id', 'unknown'), exc_info=True)
    ...
    def _exploits_vulnerable_person(self, opportunity: OffMarketOpportunity) -> bool:
        ...
        try:
            ...
            logger = structlog.get_logger(__name__)
            logger.warning(f"Failed to check vulnerability exploitation: {e}", 
                          opportunity_id=getattr(opportunity, 'id', 'unknown'), exc_info=True)
    ...
    def _has_discriminatory_bias(self, opportunities: List[OffMarketOpportunity]) -> bool:
        ...
        try:
            ...
            logger = structlog.get_logger(__name__)
            logger.warning(f"Failed to check geographic bias: {e}", exc_info=True)
            return False
