...
from ...core.exceptions import ExternalAPIError, ValidationError, AgentExecutionError
...
class CouncilDATracker:
    ...
    async def _get_suburb_market_conditions(self, suburb: str) -> float:
        ...
        try:
            ...
            logger.warning(f"Failed to calculate suburb strength: {e}", suburb=suburb, exc_info=True)
    ...
    def _calculate_da_opportunity_expiry(self, da_record: Dict[str, Any]) -> Optional[datetime]:
        ...
        try:
            ...
            logger = structlog.get_logger(__name__)
            logger.warning(f"Failed to calculate DA expiry date: {e}", da_number=da_record.get('da_number'), exc_info=True)
            return datetime.utcnow() + timedelta(days=365)  # Safe default
        except Exception as e:
            logger = structlog.get_logger(__name__)
            logger.error(f"Unexpected error calculating DA expiry: {e}", da_number=da_record.get('da_number'), exc_info=True)
            raise ValidationError("da_expiry", da_record, f"DA expiry calculation failed: {e}") from e
