...
class MultiAgentOrchestrator:
    ...
    async def _call_agent(self, agent_name: str, input_data: Dict[str, Any], timeout_seconds: int) -> Dict[str, Any]:
        """Execute real agent instead of returning mock responses."""
        
        start_time = time.time()
        
        try:
            # Import and instantiate the actual agent
            agent_class = self._get_agent_class(agent_name)
            agent_instance = agent_class()
            
            # Execute the agent with real input data
            agent_result = await asyncio.wait_for(
                agent_instance.execute(input_data),
                timeout=timeout_seconds
            )
            
            # Adapt agent result to orchestrator expected format
            return self._adapt_agent_response(agent_name, agent_result, start_time)
            
        except asyncio.TimeoutError:
            execution_time = time.time() - start_time
            if self.logger:
                self.logger.warning(f"Real agent execution timeout for {agent_name} after {execution_time:.2f}s")
            return {
                "success": False,
                "error": f"Agent {agent_name} execution timed out after {timeout_seconds} seconds",
                "execution_time": execution_time,
                "confidence": 0.0,
                "data_sources": []
            }
            ...
            execution_time = time.time() - start_time
            if self.logger:
                self.logger.error(f"Real agent execution failed for {agent_name}: {e}")
            return {
                "success": False,
                "error": str(e),
                "execution_time": execution_time,
    ...
    def _get_agent_class(self, agent_name: str):
        """Map agent names to actual agent classes and import them dynamically."""
        
        agent_mapping = {
            "listing_watcher": "src.agents.listing_watcher.agent.ListingWatcherAgent",
            "suburb_signal": "src.agents.suburb_signal.agent.SuburbSignalAgent", 
            "buyer_matchmaker": "src.agents.buyer_matchmaker.agent.BuyerMatchmakerAgent",
            "seller_strategy": "src.agents.seller_strategy.agent.SellerStrategyAgent",
            "off_market_radar": "src.agents.off_market_radar.agent.OffMarketRadarAgent"
        }
        ...
        try:
            ...
            raise ImportError(f"Failed to import agent {agent_name}: {e}")
        except AttributeError as e:
            raise AttributeError(f"Agent class {class_name} not found in {module_path}: {e}")
    
    def _adapt_agent_response(self, agent_name: str, agent_result: Dict[str, Any], start_time: float) -> Dict[str, Any]:
        """Adapt agent response to orchestrator expected format."""
        ...
        if isinstance(agent_result, dict):
            ...
            data = agent_result.get("data", agent_result.get("result", {}))
            confidence = agent_result.get("confidence", agent_result.get("confidence_score", 0.8))
            data_sources = agent_result.get("data_sources", [f"{agent_name}_execution"])
            ...
            return {
                ...
                "confidence": 0.8,
                "data_sources": [f"{agent_name}_execution"],
                "execution_time": execution_time,
                "agent_name": agent_name,
                "real_execution": True
            }
