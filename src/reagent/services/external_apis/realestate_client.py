...
realestate_api_breaker = CircuitBreaker(fail_max=5, reset_timeout=60)


from reagent.core.exceptions import (
    ExternalAPIError as RealEstateAPIError,
    APIRateLimitError as RealEstateRateLimitError,
    AuthenticationError as RealEstateAuthenticationError,
    DataIntegrityError as RealEstateDataError
)
...
class RealEstateAPIClient:
    ...
    @track_external_api_call(api_name="realestate", endpoint="listings/residential/_search")
    async def _make_request(
        ...
        if cache_key and method.upper() == "GET":
            ...
            if cached_item:
                ...
                # Track cache hit
                track_external_api_call(api_name="realestate", endpoint=endpoint, cache_hit=True)
    ...
    async def _handle_response(
        ...
        if cache_key and response.status == 200:
            ...
            logger.debug("RealEstate API response cached", cache_key=cache_key, etag=etag)
